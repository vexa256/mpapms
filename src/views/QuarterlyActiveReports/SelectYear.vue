        ...
        <div class="card-header pt-7 mb-5">
          ...
          <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bold text-gray-800"
              >Select the quarterly reports are required
            </span>
            <span class="text-gray-500 mt-1 fw-semibold fs-6"
              >Only quarterly reports with reported indicators are shown</span
            >
          </h3>
          <!--end::Title-->
    ...
    async fetchEntities() {
      ...
      try {
        const data = await pb
          ...
          .getFullList({
            filter: `Entity != "RRF" && EntityID = "${this.$route.query.entityid}"`,
          });
