      ...
      <div class="card-body pt-3 fw-bolder text-white shadow-lg table-responsive">
        <div class="alert alert-info shadow-lg">
          ...
          <div class="d-flex flex-column">
            <h4 class="mb-1 text-dark">Select Bi-Annual Report</h4>
            <span>Select a bi annual report you want to file. All reporting actions will be attached to this report.</span>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchReports() {
        ...
        try {
          const data = await pb.collection('mpa_reporting_timelines')
            .getFullList({
              filter: `status="active" && Type="Bi-Annual Report"`
            });
          this.reports = data;
        } catch (error) {
